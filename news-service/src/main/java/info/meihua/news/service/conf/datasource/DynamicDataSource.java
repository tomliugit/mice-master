...
public class DynamicDataSource extends DataSource {
    ...
    public Connection getConnection() {
        String projectDbInfo = DbIdentifier.getProjectDb();

        String projectCode = projectDbInfo.split("#")[0];

        Integer env = Integer.valueOf(projectDbInfo.split("#")[1]);

        Boolean read = Boolean.valueOf(projectDbInfo.split("#")[2]);
        ...
        if (dds == null) {
            try {
                DataSource newDDS = initDDS(projectCode, env, read);
                DdsHolder.instance().addDDS(projectCode, newDDS);
            } catch (IllegalArgumentException | IllegalAccessException e) {
                logger.info("Init data source fail. projectCode:" + projectCode);
                return null;
            }
        }
     ...
     */
    private DataSource initDDS(String projectCode, Integer env, Boolean read) throws IllegalArgumentException, IllegalAccessException {
        ...
        String url = String.format(urlFormat, GlobalConstants.Host.dbIp(env, read),
                projectCode);
        dds.setUrl(url);
